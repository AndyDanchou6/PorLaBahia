            ...
            if (accommodation.length > 0) {
                accommodation.forEach((acc, index) => {
                    const accommodationText = $(
                        `.accommodation${index + 1}Details`
                    );
                    const sliderImagesContainer = $(
                        `.accommodationSlider${index + 1}`
                    );
                    const accommodationList = $(
                        `.accommodation${index + 1}Image`
                    );
                    ...
                    const accommodationDetails = `
                        <div class="accommodationDetails">
                            ...
                            <div class="price-book${index + 1}">
                                <div class="accommodationPrice${index + 1}">₱ ${
                        acc.weekday_price
                    }</div>
                                <button class="accommodationBook${
                                    index + 1
                                }"><img src="/images/icon.svg" alt="">Book This</button>
                            </div>
                    ...
                    accommodation.forEach((acc, index) => {
                        ...
                        if (acc.galleries && acc.galleries.length > 0) {
                            ...
                            if (matchingGalleries.length > 0) {
                                ...
                                sliderImagesContainer.append(
                                    "<p><i>No other images available for this accommodation.</i></p>"
                                );
                            }
                        } else {
                            sliderImagesContainer.append(
                    ...
                    $(document).on(
                        "click",
                        `.sliderImage${index + 1} img`,
                        function () {
                            const selectedImageSrc = $(this).attr("src");
                            $(`.accommodation${index + 1}Image`).html(
                                `<div class="image${
                                    index + 1
                                }Container"><img src="${selectedImageSrc}" alt=""></div>`
                            );
                            $(`.sliderImage${index + 1} img`).removeClass(
                                "active"
                            );
